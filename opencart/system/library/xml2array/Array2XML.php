...
class Array2XML
{
	...
	public function &buildXml($array)
	{
		...
		if (array_key_exists('rootNodeName', $this->config) && is_string($this->config['rootNodeName'])) {
			...
			if (is_array($array) && count($array) == 1) {
				//$rootNodeName = array_key_first($array);
                reset($array);
                $rootNodeName = key($array);
				$array = $array[$rootNodeName];
			}
		}

		if ($rootNodeName === null) {
			throw new \Exception('[Array2XML] Root node name could not be determined');
		}
	 ...
	 * @param array|string|int|bool $array The array or value to be converted
		...
		if (is_array($array)) {
			...
			if (isset($array[$this->config['attributesKey']])) {
				foreach ($array[$this->config['attributesKey']] as $key => $value) {
                    ...
                    if(is_array($this->bool2str($value))) {
                        if(!empty($value['@cdata']))
                            $value = $value['@cdata'];
                    }
	...
	/**
	 * Check if the array has numeric keys
	 * @param array $array The array to check
	 * @return bool True if the array has numeric keys
	 */
	protected function isNumericArray($array)
	{
		...
		// Check if the array keys are numeric
