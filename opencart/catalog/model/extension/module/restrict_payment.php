...
//namespace Opencart\Catalog\Model\Extension\RestrictPayment\Module;
//class RestrictPayment extends \Opencart\System\Engine\Model {

class ModelExtensionModuleRestrictPayment extends Model {
	...
	public function restrict($extensions) {
		...
		*/
		...
		if (version_compare(VERSION, '4.0', '<')) {
			...
			if (isset($this->session->data['shipping_method']) && isset($this->session->data['shipping_methods']) && isset($this->session->data['shipping_methods'][$shipping_method[0]])) {
				$shipping_rate = $this->session->data['shipping_methods'][$shipping_method[0]]['quote'][$shipping_method[1]]['title'];
				$shipping_cost = $this->session->data['shipping_methods'][$shipping_method[0]]['quote'][$shipping_method[1]]['cost'];
			} else {
				$shipping_rate = '';
				$shipping_cost = 0;
			}
			...
			$shipping_method = (isset($this->session->data['shipping_method']['code'])) ? substr($this->session->data['shipping_method']['code'], 0, strpos($this->session->data['shipping_method']['code'], '.')) : '';
			$shipping_rate = (isset($this->session->data['shipping_method']['name'])) ? strtolower($this->session->data['shipping_method']['name']) : '';
			$shipping_cost = (isset($this->session->data['shipping_method']['cost'])) ? $this->session->data['shipping_method']['cost'] : 0;
		}
		...
		foreach ($settings['restriction'] as $row) {
			...
			$row_disabled_text = 'Disabled [' . strtoupper($row['methods']) . ']';
				...
				foreach ($rules['product_group'] as $comparison => $product_group_ids) {
					...
					foreach ($product_group_ids as $product_group_id) {
						...
						foreach ($members_array as $type => $members) {
							...
							foreach ($cart_products as $product) {
								if ($this->type == 'shipping' && !$product['shipping'] || empty(${$type.'s'}[$product['key']])) {
									continue;
								}
				...
				if (isset($settings['distance_calculation']) && $settings['distance_calculation'] == 'driving') {
					...
					if (empty($directions['routes'])) {
						...
						if (empty($directions['routes'])) {
							...
							$this->logMessage('The Google directions service returned the error "' . $google_error . '" for origin "' . $store_address_string . '" and destination "' . $customer_address_string . '"');
							continue;
						}
					}
					...
					if ($this->config->get('config_geocode')) {
						...
						if (empty($geocode['results'])) {
							...
							if (empty($geocode['results'])) {
								...
								$this->logMessage('The Google geocoding service returned the error "' . $google_error . '" for address "' . $store_address_string . '"');
								continue;
							}
						}
					...
					if (!empty($address['geocode'])) {
						...
						if (empty($geocode['results'])) {
							...
							if (empty($geocode['results'])) {
								$google_error = (!empty($geocode['status']) ? $geocode['status'] : '') . (!empty($geocode['error_message']) ? ': ' . $geocode['error_message'] : '');
								$this->logMessage('The Google geocoding service returned the error "' . $google_error . '" for address "' . $customer_address_string . '"');
								continue;
							}
						}
			...
			$this->logMessage('All rules passed, enabling [' . strtoupper($row['methods']) . ']');
	...
	private function ruleViolation($rule, $value, $product_name = '') {
		...
		if ($violation) {
			if ($product_name) {
				...
				$this->logMessage('Disabled [' . strtoupper($this->row['methods']) . '] for violating rule "' . $rule . ' ' . $comparison . ' ' . implode(', ', $rules[$rule][$comparison]) . '" with value "' . (is_array($value) ? implode(',', $value) : $value) . '"');
			}
		}
	...
	private function inRange($value, $range_list, $charge_type = '', $skip_testing = false) {
		...
		if (!$skip_testing) {
			if (strpos($charge_type, ' not') ? $in_range : !$in_range) {
				$this->logMessage('Disabled [' . strtoupper($this->row['methods']) . '] for violating rule "' . $charge_type . (strpos($charge_type, ' not') ? ' ' : ' is ') . implode(', ', $range_list) . '" with value "' . $value . '"');
			}
		}
