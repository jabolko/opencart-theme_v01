...
class ControllerCheckoutCheckout extends Controller {
	...
	public function index() {
		...
		// Cart summary for total bar
		$cart_products = $this->cart->getProducts();
		$cart_count = 0;
		$cart_subtotal = 0;

		foreach ($cart_products as $cp) {
			$cart_count += $cp['quantity'];
			$cart_subtotal += $this->tax->calculate($cp['price'], $cp['tax_class_id'], $this->config->get('config_tax')) * $cp['quantity'];
		}

		$data['cart_count'] = $cart_count;
		$data['cart_total'] = $this->currency->format($cart_subtotal, $this->session->data['currency']);
